...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    ...
    ////////////////////////////////////////////////////
    //        CHECK HERE FOR CHANGES BY ME :-)        //
    ////////////////////////////////////////////////////
    // added this function so it can call changePizzaSizes
...
////////////////////////////////////////////////////
  ...
  function changePizzaSizes(size) {
    ...
    var randomPizza = document.querySelectorAll(".randomPizzaContainer"),
        randomPizzaLength = randomPizza.length;
...
function updatePositions() {
  ...
  // Took out all variables that are not reliant on i
  // to function this way helping with performance
  // by reading layout properties first and storing them
  var items = document.querySelectorAll('.mover'),
      itemsLength = items.length,
      scrollTop = document.body.scrollTop / 1250,
      phase = [];

  // pushing calculations for the phase into a array
  for(var x = 0; x < itemsLength; x++) {
    phase.push(Math.sin(scrollTop + (x % 5)));
  }

  // using translateX instead of style.left since translate is alot better
  // on performance then style.left
  for (var i = 0, phaseCount = 0; i < itemsLength; i++, phaseCount++) {
    var pixels = -1200 + (items[i].basicLeft + 100 * phase[phaseCount]) + 'px';
    items[i].style.transform = 'translateX(' + pixels + ')';
...
// added a requestAnimationFrame to help keep animation fluid
window.addEventListener('scroll', function() {
  window.requestAnimationFrame(updatePositions);
});

var movingPizzasItem = document.querySelector("#movingPizzas1");

// Generates the sliding pizzas when the page loads.
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //  changed the amount of sliding pizzas
  for (var i = 0; i < 35; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "img/moving.png";
    elem.basicLeft = (i % cols) * s;
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    movingPizzasItem.appendChild(elem);
  }
  updatePositions();
});
